...
const { RESOURCE_CONFIG, LEAGUE_TIERS, LEAGUE_REWARDS, ALL_ROLES } = require('../../config/game-config');
const {
...
// GET /api/league/tiers - 获取联赛梯度列表及剩余次数
router.get('/tiers', authCheck, (req, res) => {
  const db = getDb();
  const today = getTodayStr();

  const tiers = LEAGUE_TIERS.map(tier => {
    // 查询玩家该梯度今日挑战次数
    const cntResult = db.exec(
      "SELECT challenge_count FROM league_daily_challenges WHERE player_id = ? AND tier = ?",
      [req.playerId, tier.tier]
    );
    let usedCount = 0;
    if (cntResult.length > 0 && cntResult[0].values.length > 0) {
      const row = cntResult[0].values[0];
      const lastDate = db.exec(
        "SELECT last_date FROM league_daily_challenges WHERE player_id = ? AND tier = ?",
        [req.playerId, tier.tier]
      );
      if (lastDate.length > 0 && lastDate[0].values.length > 0 && lastDate[0].values[0][0] === today) {
        usedCount = row[0];
      }
    }
  ...
  res.json({ code: 0, data: { tiers } });
});

// POST /api/league/battle - 发起联赛挑战
router.post('/battle', authCheck, (req, res) => {
  ...
  // 校验梯度参数
  const tierConfig = LEAGUE_TIERS.find(t => t.tier === tier);
  if (!tierConfig) {
    return res.json({ code: 1, msg: '无效的联赛梯度' });
  }
  ...
  if (teamResult.length === 0 || teamResult[0].values.length === 0) {
    return res.json({ code: 1, msg: '联赛数据异常' });
  }
  const aiTeamId = teamResult[0].values[0][0];
  ...
  for (let i = 0; i < 3; i++) {
    allPlayerStats.push({
      side: 'defender',
      name: aiRoster[i].role_name,
      pos: aiRoster[i].pos,
      grade: aiRoster[i].grade,
      star: aiRoster[i].star,
      ...defPlayers[i]
    });
  }

  // 发放奖励（仅胜利：按梯度发放碎片+钻石）
  const tierRewards = LEAGUE_REWARDS[tier] || { shard: 0, diamond: 0 };
  let shardReward = 0;
  let diamondReward = 0;
  if (isWin) {
    shardReward = tierRewards.shard;
    diamondReward = tierRewards.diamond;
    db.run(
      "UPDATE players SET upgrade_shard = upgrade_shard + ?, diamond = diamond + ? WHERE player_id = ?",
      [shardReward, diamondReward, req.playerId]
    );
    // 黄金联赛通关成就检测
    if (tier === 'gold') {
      finishAchievement(req.playerId, 'gold_league_clear');
    }
  }
  ...
  const myInfo = db.exec(
    "SELECT upgrade_shard, diamond FROM players WHERE player_id = ?",
    [req.playerId]
  );
  ...
  res.json({
    ...
    data: {
      ...
      reward: { shard: shardReward, diamond: diamondReward },
      updatedShard: updatedRow[0],
